...
struct ExportView: View {
    ...
    var body: some View {
        VStack {
            ...
            Text("Your data will be exported to your iPhone Files app. Find the file named: export.csv") // was export.json
                .frame(maxWidth: .infinity,
                       maxHeight: .infinity,
                       alignment: .center)
    ...
    private func sendDataToPhoneViaWC(dataToExport: String) {
        guard let downloadsDirUrl = FileManager.default.urls(for: .downloadsDirectory, in: .userDomainMask).first else {
            return
        }
        
        let fileUrl = downloadsDirUrl.appendingPathComponent("hrv_data.csv") // change the filename and file extension as desired
